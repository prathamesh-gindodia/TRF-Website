<?php
									...
									$tempname=$_FILES['img']['tmp_name'];
										...
										{
											$qry2="SELECT * FROM `users` WHERE `username`='$names[$i]'";
												...
												if ($row2>0)
										{
											...
											while($rows=mysqli_fetch_assoc($run3))
													{$num3++;
											$idstring=$rows['user_id'];//to fetch ids of users
										...
										{$str3=rtrim($str3,',');
											...
											$qry3="UPDATE `project` SET `id`='$id4',`title`='$title',`status`='$status',`githubLink`='$link',`teamdid`='$str3',`date`='$date',`tags`='$str4' WHERE  `id`='$id1'";
